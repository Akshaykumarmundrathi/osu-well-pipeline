...
|---|---|---|
| **Foundation** | May 15–23 | Modular pipeline: ZIP-native reading, per-record crash recovery, Vision OCR, Gemini county normalization, grid detection (OpenCV, 6 methods), first 1911-era QA set (95/100 grids detected, 94/94 hand-verified correct) |
| **Coordinate resolution** | May 23–31 | PLSS PostgreSQL database (4.56M quarter-quarter cells) on AWS RDS; 5-pass resolver: quadrant-cell → section bbox → county-constrained → direction-inferred → section centroid; bilinear interpolation of the dot position |
...
**4.5 Constraints shaped the engineering.** The 7.4 GB laptop OOM-killed every long run, including resident orchestrators. The surviving pattern: a near-zero-RAM `cmd` loop dispatching 50-record chunks as fresh processes (all memory freed on exit), idempotent via the status tracker. It processed 6,341 + 35,845-record campaigns crash-free at ~1,000 records/hour, surviving Wi-Fi losses, shutdowns, and resumes.
