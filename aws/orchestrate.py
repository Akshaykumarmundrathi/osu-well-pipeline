...
def build_s3_index(s3, slice_size: int) -> tuple[list[dict], int]:
    """
    ...
    for page in paginator.paginate(Bucket=BUCKET, Prefix=PDF_PREFIX):
        for obj in page.get("Contents", []):
            ...
            # Extract collection number from folder name
            import re
            m = re.search(r"\d+", collection)
...
def submit_pipeline_jobs(batch, total_slices: int, resume: bool) -> list[str]:
    """Submit one Batch job per slice. Returns list of job IDs."""
    prefix = "osu-pipeline-slice-"
...
def main():
    ...
    if args.stage in ("all", "pipeline"):
        if args.total_slices:
            ...
            records, total_slices = build_s3_index(s3, args.slice_size)
            upload_index(s3, records)

        submit_pipeline_jobs(batch, total_slices, args.resume)

        if not args.no_wait:
            ok = wait_for_queue(batch, "osu-pipeline-slice-", total_slices)
            if not ok:
                _p("Pipeline stage had failures — check CloudWatch logs")
